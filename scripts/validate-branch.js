...
const branchPattern = /^(feature|fix|docs|style|refactor|perf|test|chore)\/([a-zA-Z0-9-]+)\/([a-zA-Z0-9]+)\/(\d+)\/[a-z0-9-]+$/;

if (!branchPattern.test(branchName)) {
  console.error('\n❌ Branch name does not follow the project convention!');
  console.error(`\nCurrent branch: ${branchName}`);
  console.error('\n📋 Expected format:');
  console.error('   {branchType}/{projectName}/{categoryCode}/{issueNumber}/{description}');
  console.error('\n📝 Example:');
  console.error('   feature/ValidateSolution/04S1/17/document-user-id-strategy');
